...
namespace MathLib.AI
{
        public class HebbNetwork
        {
            ...
            double B = 0.1;
            public HebbNetwork(int[] layers, double learningRate)
            {
            ...
            public void Train(double[] inputs, double[] d)
            {
                ...
                for(int k=L-1;k > 0;k--)
                {
                    ...
                    for (int i = 0; i < N[k]; i++)
                    {
                        ...
                        {
                         Q[k - 1][i] += Q[k][m] * weights[k][m][i] * y[k][i] * (1 - y[k][i]);
                        }
                    }
                }
            ...
            for (int k = 0; k < L; k++)//k-number layer;    
            {
                ...
                for (int i = 0; i < N[k+1]; i++)
                {
                    ...
                    for (int j = 0; j < weights[k][i].Length; j++)
                    {
                        double d2 = Q[k][i]/ (y[k+1][i] * (1 - y[k+1][i]))+y[k+1][i];
                        weights[k][i][j] += LEARNINGRATE*d2* y[k][j];
            ...
            double sig(double v, double b)
            {
                ...
                return 1 / (1 + Math.Pow(Math.E, -b * v));
